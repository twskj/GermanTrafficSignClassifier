...
Although, the result drops dramatically on a single guess, the result of 5 guesses (top 5 prediction) does not look so bad. The model was able to correctly classify 33 out of 36 (91.7%). The top 5 was computed using `result = sess.run(topK,feed_dict={x: mySample, keep_prob: 1})` statement (Cell 16th in IPython Notebook). For the most prediction the model was too confident about its answer regardless correct and incorrect answer. Below are prediction results of the first 5 images:
